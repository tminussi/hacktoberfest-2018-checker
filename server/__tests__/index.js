...
const { fetchPullRequests, createMessage } = require('../service/github-service');
...
describe('Github service - fetchPullRequests', () => {
  it('should reject the request if status code >= 400', async () => {
    ...
    await expect(fetchPullRequests('chgasparoto')).rejects.toEqual({
      body: {},
      statusCode: 401,
    });
  });
  ...
  it('should return zero if no PR were made yet', async () => {
    ...
    const response = await fetchPullRequests('chgasparoto');
...
describe('createMessage', () => {
  it('should return a message if quantity is 0', () => {
    expect(createMessage(0)).toEqual(`You haven't created any Pull Requests so far. Start contributing to the Open Source Community and Win a T-Shirt at 5 PRs`);
  });

  it('should return a message if quantity is between 1 and 3', () => {
    expect(createMessage(2)).toEqual(`Great job. The Open Source community is already thankful to you, but you can achieve more. Keep it up!`);
  });

  it('should return a message if quantity is 4', () => {
    expect(createMessage(4)).toEqual(`You are almost there! One more to go and you will be able to wear one of the coolest T-Shirts out there`);
  });

  it('should return a message if quantity is >= 5', () => {
    expect(createMessage(5)).toEqual(`You did it! Congratulations! Time to rest and wait for your well deserved T-Shirt!`);
  });
})
